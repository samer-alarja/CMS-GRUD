...
<body>
<?php 
            $select_product = mysqli_query($link , ' SELECT name FROM  category ');
 ?>
<form class="container" method="post" action="insertartisanuser.php" enctype="multipart/form-data">
    ...
    <label for="cars">category:</label><br>
                   <?php echo '<select name="category" id="cars" style="width: 94%;">';
                    while($exc = mysqli_fetch_assoc($select_product)){
                       echo '<option value= '.$exc['name'].' name:"category">'.$exc['name'].'</option>';
                                   }
                       echo '</select>';?><br><br>
...
<select name="language" id="cars">
  ...
  <option value="other_language">other_language</option>
    ...
    &nbsp;<label>publish:</label>
    <input type="text" id="country" name="publish" value="no" readonly><br><br>

    &nbsp;<label>user_id:</label>

    <input type="text"  name="user_id" class="form-control" style="width: 500px;" value="<?php echo $_SESSION['id']?>" readonly><br>
